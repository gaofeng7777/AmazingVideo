package com.amazing.video.gp.ui.widget;
...
public class AmazingSurfaceVideoView extends SurfaceView implements Callback {
	...
	private OnAmazingPlayStateListener mOnAmazingPlayStateListener;
	...
	public AmazingSurfaceVideoView(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		initAmazingVideoView();
	}

	public AmazingSurfaceVideoView(Context context) {
		super(context);
		initAmazingVideoView();
	}

	public AmazingSurfaceVideoView(Context context, AttributeSet attrs) {
		super(context, attrs);
		initAmazingVideoView();
	}

	@SuppressWarnings("deprecation")
	protected void initAmazingVideoView() {
		//		mTryCount = 0;
		mVideoWidth = 0;
		mVideoHeight = 0;
	...
	/** 更新音量 */
	public static float getAmazingSystemVolumn(Context context) {
		if (context != null) {
			try {
	...
	public void setOnAmazingInfoListener(OnInfoListener l) {
		mOnInfoListener = l;
	}

	public void setOnAmazingVideoSizeChangedListener(OnVideoSizeChangedListener l) {
		mOnVideoSizeChangedListener = l;
	}

	public void setOnAmazingPreparedListener(MediaPlayer.OnPreparedListener l) {
		mOnPreparedListener = l;
	}

	public void setOnAmazingErrorListener(MediaPlayer.OnErrorListener l) {
		mOnErrorListener = l;
	}

	public void setOnAmazingPlayStateListener(OnAmazingPlayStateListener l) {
		mOnAmazingPlayStateListener = l;
	}

	public void setOnAmazingSeekCompleteListener(MediaPlayer.OnSeekCompleteListener l) {
		mOnSeekCompleteListener = l;
	}

	public static interface OnAmazingPlayStateListener {
		public void onStateChanged(boolean isPlaying);
	}

	public void setOnAmaingCompletionListener(MediaPlayer.OnCompletionListener l) {
		mOnCompletionListener = l;
	}

	public void setAmazingVideoPath(String path) {
	...
	public int getAmazingVideoWidth() {
		return mVideoWidth;
	}

	public int getAmazingVideoHeight() {
		return mVideoHeight;
	}

	public void reAmazingOpen() {
		mTargetState = STATE_PREPARED;
		openVideo(mUri);
	}
	...
	/** 重试 */
	private void tryAmaingAgain(Exception e) {
	...
	public void start() {
		...
		AmazingLoger.e("[SurfaceVideoView]start...1");
		if (mMediaPlayer != null && (mCurrentState == STATE_PREPARED || mCurrentState == STATE_PAUSED || mCurrentState == STATE_PLAYING || mCurrentState == STATE_PLAYBACK_COMPLETED)) {
			try {
				if (!isPlaying())
					mMediaPlayer.start();
				AmazingLoger.e("[SurfaceVideoView]start...2");
				mCurrentState = STATE_PLAYING;
				if (mOnAmazingPlayStateListener != null)
					mOnAmazingPlayStateListener.onStateChanged(true);
	...
	public void pause() {
		...
		if (mMediaPlayer != null && (mCurrentState == STATE_PLAYING)) {
			try {
				...
				if (mOnAmazingPlayStateListener != null)
					mOnAmazingPlayStateListener.onStateChanged(false);
			} catch (IllegalStateException e) {
				tryAmaingAgain(e);
			} catch (Exception e) {
				tryAmaingAgain(e);
	...
	/** 处理音量键 */
	public void dispatchAmazimngKeyEvent(Context context, KeyEvent event) {
		switch (event.getKeyCode()) {
		case KeyEvent.KEYCODE_VOLUME_DOWN:
		case KeyEvent.KEYCODE_VOLUME_UP:
			setAmazingVolume(getAmazingSystemVolumn(context));
			break;
		}
	}

	public void setAmazingVolume(float volume) {
	...
	public void setAmazingLooping(boolean looping) {
		//可用状态{Idle, Initialized, Stopped, Prepared, Started, Paused, PlaybackCompleted}
		if (mMediaPlayer != null && (mCurrentState == STATE_PREPARED || mCurrentState == STATE_PLAYING || mCurrentState == STATE_PAUSED || mCurrentState == STATE_PLAYBACK_COMPLETED)) {
			try {
	...
	/** 获取当前播放位置 */
	public int getAmazingCurrentPosition() {
		int position = 0;
	...
	public SurfaceHolder getAmazingSurfaceHolder() {
	...
	private MediaPlayer.OnErrorListener mErrorListener = new MediaPlayer.OnErrorListener() {
		...
		public boolean onError(MediaPlayer mp, int framework_err, int impl_err) {
			AmazingLoger.e("[VideoView]Error:" + framework_err + "," + impl_err);
			mCurrentState = STATE_ERROR;
	...
	/** 定时暂停 */
	public void pauseAmazingDelayed(int delayMillis) {
	...
	public void loopAmazingDelayed(int startTime, int endTime) {
		...
		if (mVideoHandler.hasMessages(HANDLER_MESSAGE_LOOP))
			mVideoHandler.removeMessages(HANDLER_MESSAGE_LOOP);
		mVideoHandler.sendMessageDelayed(mVideoHandler.obtainMessage(HANDLER_MESSAGE_LOOP, getAmazingCurrentPosition(), delayMillis), delayMillis);
	}
