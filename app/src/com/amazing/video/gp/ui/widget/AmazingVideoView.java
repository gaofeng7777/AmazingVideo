...
public class AmazingVideoView extends TextureView implements android.view.TextureView.SurfaceTextureListener {
	...
	public AmazingVideoView(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		initVideoView();
	}

	public AmazingVideoView(Context context) {
		super(context);
		initVideoView();
	}

	public AmazingVideoView(Context context, AttributeSet attrs) {
		super(context, attrs);
		initVideoView();
	}
...
//		}
	...
	public void setAmaizngLooping(boolean looping) {
		//可用状态{Idle, Initialized, Stopped, Prepared, Started, Paused, PlaybackCompleted}
		if (mMediaPlayer != null && (mCurrentState == STATE_PREPARED || mCurrentState == STATE_PLAYING || mCurrentState == STATE_PAUSED || mCurrentState == STATE_PLAYBACK_COMPLETED)) {
			try {
	...
	public void seekAmazingTo(int msec) {
		//可用状态{Prepared, Started, Paused, PlaybackCompleted}
		if (mMediaPlayer != null && (mCurrentState == STATE_PREPARED || mCurrentState == STATE_PLAYING || mCurrentState == STATE_PAUSED || mCurrentState == STATE_PLAYBACK_COMPLETED)) {
			try {
				if (msec < 0)
	...
	public void openVideo(Uri uri) {
		...
		AmazingLoger.e("[LocalVideoView]openVideo...");
	...
	/** 是否可用 */
	public boolean isAmzaingPrepared() {
	...
	/** 暂停并且清除定时任务 */
	public void pauseAmaingClearDelayed() {
		pause();
		if (mVideoHandler.hasMessages(HANDLER_MESSAGE_PARSE))
			mVideoHandler.removeMessages(HANDLER_MESSAGE_PARSE);
	...
	public void loopAmazingDelayed(int startTime, int endTime) {
		...
		seekAmazingTo(startTime);
		if (!isPlaying())
			start();
		if (mVideoHandler.hasMessages(HANDLER_MESSAGE_LOOP))
			mVideoHandler.removeMessages(HANDLER_MESSAGE_LOOP);
		mVideoHandler.sendMessageDelayed(mVideoHandler.obtainMessage(HANDLER_MESSAGE_LOOP, getAmaingCurrentPosition(), delayMillis), delayMillis);
	}
}
