package com.amazing.video.gp.important;
...
public class AmazingImportantThemeSingle implements Serializable {
	/** 音乐（服务端定义） */
	...
	public List<AmazingImportantThemeSingle> items = new ArrayList<AmazingImportantThemeSingle>();

	public AmazingImportantThemeSingle() {

	}

	/** 用于解析在线主题 */
	public AmazingImportantThemeSingle(JSONObject jst, int themeType) {
		...
		price = AmazingConvertToUtils.toAmazingInt(jst.optString("price"));
		previewVideoPath = jst.optString("channel");
		themeDownloadUrl = jst.optString("downurl");
		themeUpdateAt = AmazingConvertToUtils.toAmazingLong(jst.optString("update_at"));
	...
	/** 更新主题 */
	public void update(AmazingImportantThemeSingle theme) {
		if (theme == null) {
			return;
		}
	 ...
	 *            default null
	 */
	public AmazingImportantThemeSingle(JSONObject jst, String bannerTheme) {
		sthid = jst.optString("content");
		banner = jst.optString("pic");
	}

	/** 用于解析本地json */
	public AmazingImportantThemeSingle(JSONObject obj) {
	...
	/** 检测是否是内置的主题 */
	public boolean amazingisNestMusic() {
		return StringUtils.isEmpty(themeDownloadUrl);
	}

	/** 检测是否是变声 */
	public boolean amazingIsSoundEffect() {
		return soundType > 0;
	}

	/** 是否是mv主题 */
	public boolean amazingIsMV() {
		return isMv;
	}
	...
	/** 获取水印路径 */
	public String getAmazingFilterPath() {
		return FileUtils.concatPath(themeFolder, themeName + fileExt);
	}

	/** 是否要生成文字 */
	public boolean isAmazingText() {
		return textSize > 0;
	}
	...
	public String toString() {
		return String.format("themeName:%s isMv:%s lockType:%d isCity:%s",
				themeName, amazingIsMV() ? "true" : "false", lockType, isCity ? "true"
						: "false");
	}
}
