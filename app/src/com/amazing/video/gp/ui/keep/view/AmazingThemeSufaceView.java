...
import com.amazing.video.gp.ui.keep.coagent.AmazingRecorderHelper;
 ...
 *
 */
public class AmazingThemeSufaceView extends SurfaceView implements Callback {
	...
	public AmazingThemeSufaceView(Context context, AttributeSet attrs) {
		super(context, attrs);
		init();
	}

	public AmazingThemeSufaceView(Context context) {
		super(context);
		init();
	}

	public void amazingReset() {
		mFilterPath = "";
		mWatermarkPath = "";
		mWatermarkBlendmode = "";
		mSpeed = 1F;
	...
	public void setAmazingPoster(String path) {
		this.mPoster = path;
	}

	public void setAmazingSpeed(float speed) {
		this.mSpeed = speed;
	}

	/** 设置主题静音 */
	public void setAmazingThemeMute(boolean mute) {
		this.mThemeMute = mute;
	}

	/** 设置原声静音 */
	public void setAmazingOrgiMute(boolean mute) {
		this.mOrgiMute = mute;
	}
	...
	public void setIntent(Intent intent) {
		if (intent != null) {
			mImportImage = intent.getBooleanExtra(AmazingCommonIntentExtra.EXTRA_MEDIA_IMPORT_IMAGE, false);
			boolean mImportVideo = intent.getBooleanExtra(AmazingCommonIntentExtra.EXTRA_MEDIA_IMPORT_VIDEO, false);
...
//			}
	...
	/** 设置视频存储信息 */
	public void setAmazingMediaObject(MediaObject obj) {
		this.mMediaObject = obj;
	}

	/** 传递当前主题信息 */
	public void setAmazingTheme(AmazingImportantThemeSingle theme) {
		this.mCurrentTheme = theme;
	}

	/** 设置mp4路径 */
	public void setAmazingOutputPath(String output) {
		this.mOutputPath = output;
	}

	/** 设置输入源 */
	public void setAmazingInputPath(String input) {
		this.mInputPath = input;
	}

	/** MV滤镜路径 */
	public void setAmazingMVPath(String path) {
		this.mMVPath = path;
	}

	/** 设置高级编辑-滤镜 */
	public void setAmazingFilterPath(String path) {
		this.mFilterPath = path;
	}

	/** 设置高级编辑-水印 */
	public void setAmaziangWatermarkPath(String path, String blendmode) {
		this.mWatermarkPath = path;
		this.mWatermarkBlendmode = blendmode;
	}

	/** MV滤镜公共路径 */
	public void setAmazingFilterCommomPath(String path) {
		this.mFilterCommonPath = path;
	}

	/** 设置片尾动画路径 */
	public void setAmazingVideoEndPath(String path) {
		this.mEndPath = path;
		this.mEndWidth = AmazingPreferenceUtils.getInt(AmazingPreferenceKeys.THEME_LOGO_AUTHOR_WIDTH, 0);
		this.mEndHeight = AmazingPreferenceUtils.getInt(AmazingPreferenceKeys.THEME_LOGO_AUTHOR_HEIGHT, 0);
	}
	...
	public void surfaceCreated(SurfaceHolder holder) {
		...
		if (mNeedInit)
			initFilter();
		AmazingLoger.e("[ThemeSurfaceView]surfaceCreated...");
	}

	@Override
	public void surfaceChanged(SurfaceHolder holder, int format, int width, int height) {
		mSurfaceHolder = holder;
		AmazingLoger.e("[ThemeSurfaceView]surfaceChanged...");
	}
	...
	public void surfaceDestroyed(SurfaceHolder holder) {
		...
		AmazingLoger.e("[ThemeSurfaceView]surfaceDestroyed...");
	}
	 ...
	 * @return
	 */
	public boolean startAmazingEncoding() {
	...
	/** 暂停并且清除定时任务 */
	public void pauseAndClearDelayed() {
		pause();
		clearAmazingMessages();
	}

	/** 区域内循环播放 */
	public void loopAmazingDelayed() {
		if (mIsPause)
			start();
		else
			initFilter();
	...
	private void clearAmazingMessages() {
	...
	private String getSetting(boolean output) {
		...
		if (mMediaObject != null) {
			...
			settings.append(AmazingRecorderHelper.getAmazingVideoBitrate());//mMediaObject.mVideoBitrate
			settings.append("; ");
		}
		//随机因子
