...
import com.amazing.video.gp.ui.keep.AmazingMediaPreviewActivity;
 ...
 *
 */
public class MainActivity extends AmazingBaseActivity implements
	...
	private void loadIntent() {
		...
		mFocusWidth = AmazingConvertToUtils.dipToPX(this, 64);
	...
	private void loadAmazingViews() {
		...
		initAmazingSurfaceView();
	}

	/** 初始化画布 */
	private void initAmazingSurfaceView() {
	...
	/** 初始化拍摄SDK */
	private void initAmazingMediaRecorder() {
	...
	private View.OnTouchListener mOnSurfaveViewTouchListener = new View.OnTouchListener() {
		...
		public boolean onTouch(View v, MotionEvent event) {
			...
			case MotionEvent.ACTION_DOWN:
				// 检测是否手动对焦
				if (checkAmazingCameraFocus(event))
					return true;
				break;
			}
			return true;
	...
	private boolean checkAmazingCameraFocus(MotionEvent event) {
	...
	/** 开始录制 */
	private void startAmazingRecord() {
		if (mMediaRecorder != null) {
	...
	public void onBackPressed() {
		...
		if (mMediaObject != null)
			mMediaObject.delete();
		finish();
		overridePendingTransition(R.anim.amazing_push_bottom_in, R.anim.amazing_push_bottom_out);
	}

	/** 停止录制 */
	private void stopAmazingRecord() {
	...
	/** 取消回删 */
	private boolean cancelAmazingDelete() {
		if (mMediaObject != null) {
			MediaObject.MediaPart part = mMediaObject.getCurrentPart();
	...
	/** 检查录制时间，显示/隐藏下一步按钮 */
	private int checkAmazingStatus() {
		int duration = 0;
	...
	public void onEncodeStart() {
		showAmazingProgress("", getString(R.string.record_camera_progress_message));
	}

	@Override
	public void onEncodeProgress(int progress) {
		AmazingLoger.e("[MediaRecorderActivity]onEncodeProgress..." + progress);
	}
	...
	public void onEncodeComplete() {
		hideAmazingProgress();
		Intent intent = new Intent(this, AmazingMediaPreviewActivity.class);
		Bundle bundle = getIntent().getExtras();
		if (bundle == null)
			bundle = new Bundle();
		bundle.putSerializable(AmazingCommonIntentExtra.EXTRA_MEDIA_OBJECT,
				mMediaObject);
